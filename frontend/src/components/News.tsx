...
const News: React.FC = () => {
  ...
  return (
    <motion.section
      className={`news-feed ${isNewsPage ? 'mx-auto w-full' : 'w-1/5'} flex flex-col max-h-[91vh] pt-4 bg-dark text-text`}
      ...
      {showNews && (
        <div className={`grid ${isNewsPage ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1' : ''} overflow-y-scroll custom-scrollbar pr-2`}>
          {news.filter(item => item.data.content).map((item) => (
            <div key={item.id} className={`${isNewsPage ? `m-4 ` : "mb-4 max-w-[400px]"} p-4 bg-dark rounded inline-flex flex-col min-w-[200px] text-text`}>
